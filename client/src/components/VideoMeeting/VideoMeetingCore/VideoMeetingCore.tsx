            ...
            <Paper padding="lg" elevation={3}>
              <Stack spacing="lg">
                  ...
                  <Stack spacing="md">
                    ...
                    <Box>
                      ...
                      <input 
                        ...
                        style={{ padding: '8px', width: '100%', borderRadius: '4px', border: '1px solid var(--color-border-default)' }}
                      />
                    </Box>
                  </Stack>
                  <Stack spacing="md">
                    <Typography variant="body-small" fontWeight={600}>Invite Participants</Typography>
                    <Box style={{ maxHeight: '200px', overflowY: 'auto', border: '1px solid var(--color-border-default)', borderRadius: '4px' }}>
                      <Typography variant="caption" padding="xs" style={{ backgroundColor: 'var(--color-bg-tertiary)', display: 'block' }}>Users</Typography>
                      {userList.map(u => (
                      ...
                      <Typography variant="caption" padding="xs" style={{ backgroundColor: 'var(--color-bg-tertiary)', display: 'block' }}>Organizations</Typography>
                      {orgList.map(o => (
                        <Flex key={o._id} padding="xs" align="center" gap="sm">
                          ...
                          />
                          <Typography variant="body-small">{o.name} ({o.dept1})</Typography>
                        </Flex>
                      ))}
                    </Box>
                  </Stack>
                ...
                <Flex justify="flex-end" gap="md">
                  ...
                  <Button variant="primary" onClick={handleScheduleMeeting} disabled={!newMeeting.title || !newMeeting.scheduledAt}>
                    Save Schedule
                  </Button>
                </Flex>
              </Stack>
          ...
          {showCreateForm && (
            <Paper padding="lg" elevation={3}>
              <Stack spacing="lg">
                ...
                <Flex gap="sm">
                  {(['General', 'Web', 'Design'] as Category[]).map((cat) => (
                    <Button
                      key={cat}
                ...
                />
                <Flex justify="flex-end" gap="md">
                  <Button variant="ghost" onClick={() => store.setShowCreateForm(false)}>Cancel</Button>
                  <Button variant="primary" onClick={handleCreateRoom} disabled={!roomTitle.trim()}>Create & Join</Button>
                </Flex>
              </Stack>
            </Paper>
          )}
          ...
          <Grid columns="1fr 1fr" gap="xl">
            ...
            <Stack spacing="md">
              ...
              {scheduledMeetings.length === 0 ? (
                <Paper padding="lg" variant="outlined"><Typography align="center" color="text-secondary">No scheduled meetings.</Typography></Paper>
              ) : (
                scheduledMeetings.map(m => (
                  <Card key={m._id} variant="outlined">
                    ...
                    <CardBody>
                      ...
                      <Typography variant="caption" style={{ marginTop: '8px', display: 'block' }}>Host: {m.hostId.username}</Typography>
                    </CardBody>
                    <CardFooter>
                      <Button fullWidth variant={m.status === 'ongoing' ? 'primary' : 'secondary'} disabled={m.status === 'cancelled'}>
            ...
            <Stack spacing="md">
              ...
              {roomList.length === 0 ? (
                <Paper padding="lg" variant="outlined"><Typography align="center" color="text-secondary">No live public rooms.</Typography></Paper>
              ) : (
                roomList.map((room) => (
                  <Card key={room.roomId} variant="outlined">
                    <CardHeader>
                      ...
                      <Typography variant="h4" style={{ marginTop: '8px' }}>{room.title}</Typography>
                    </CardHeader>
                    <CardFooter>
                      <Button fullWidth onClick={() => handleJoinRoom(room)}>Join Room</Button>
                    </CardFooter>
                  </Card>
                ))
              )}
                ...
                {pendingRequests.map((req) => (
                  <Paper key={req.socketId} variant="outlined" padding="sm">
                    ...
                    <Flex gap="sm" style={{ marginTop: '8px' }}>
                      <Button size="sm" onClick={() => store.approveRequest(req.socketId)} fullWidth>Accept</Button>
                      <Button size="sm" variant="secondary" onClick={() => store.rejectRequest(req.socketId)} fullWidth>Reject</Button>
                    </Flex>
                  </Paper>
                ))}
              </Stack>
